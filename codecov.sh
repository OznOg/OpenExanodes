...
then
  ...
  then
    ...
    then
      ...
      find ~/Library/Developer/Xcode/DerivedData -name '*.gcda' -exec gcov -pbcu {} +

      profdata=$(find ~/Library/Developer/Xcode/DerivedData -name '*.profdata' | head -1)
      if [ -f "$profdata" ];
      then
        swiftcov "$profdata"
      else
    ...
    if [ "$derivedDataPath" != "" ];
    then
      profdata=$(find "$derivedDataPath" -name '*.profdata' | head -1)
      if [ -f "$profdata" ];
      then
        swiftcov "$profdata"
      else
    ...
    pwd
    find $proj_root -type f -name '*.gcno' $gcov_ignore -exec $gcov_exe -pb $gcovargs {} +
