...
sub usage
{
  ...
  printf(".$HIGH Usage$NORM : fusion_clstart CLUSTERNAME\n");
  ...
  printf("$HIGH\tdémarrage$NORM des espaces virtuels et des volumes logiques.\n");
...
# void startgroups(char *gname[]);
#
# lance les groupes donnés en paramètre
sub startgroups {
  foreach $group (@groupslist) {
    PSYSTEM("$constantes::DGSTARTCMD $debugopt $group");
  }
}
